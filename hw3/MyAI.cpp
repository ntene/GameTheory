...
{	
	...
	std::random_device rd;
 	...
 	for(int i = 0; i < 32; i++){
 		for(int j = 1; j < 8; j++){
 			Random_number[i][j][RED] = distribution(generator);
 			Random_number[i][j][BLACK] = distribution(generator);
 		}
 		Random_number[i][0][RED] =  Random_number[i][0][BLACK] = distribution(generator);
 		bit_board.Counter_hash = (bit_board.Counter_hash^Random_number[i][0][0]);
 	}
 	Random_color[RED] = distribution(generator);
 	Random_color[BLACK] = distribution(generator);
 	Hash_table.clear();
...
void MyAI::generateMove(char move[6])
{
	...
	double t = 0;
	if (this->Color == 2){
		int initial_move[] = {4, 7, 24, 27};
		startPoint = EndPoint = initial_move[rand()%3];
	}
	else{
		t = Nega_scout(this->Board, this-> bit_board, &best_move, this->Red_Chess_Num, this->Black_Chess_Num, this->CoverChess, this->Color, -DBL_MAX, DBL_MAX, 0, DEPTH_LIMIT);
		int Temp_Result[2048];
		int move_count;
		if (best_move == -1){
			move_count =  Expand_bitboard(this-> bit_board, this->Color, Temp_Result);
			uint32_t covered_chess = bit_board.piece[15];
			while(covered_chess){
				uint32_t mask = LSB(covered_chess);
				covered_chess ^= mask;
				int result = GetIndex(mask);
				Temp_Result[move_count] = result*32+result;
				move_count++;
			}
			best_move = Temp_Result[rand() % move_count];
			t = 0;
			printf("SOMETHING IS WRONG\n");
		}
		fprintf(stderr,"Expand = %d\n\n", Expand_bitboard(this-> bit_board, this->Color, Temp_Result));
		//fprintf(stderr, "red chess num = %d black chess num = %d\n",Red_Chess_Num, Black_Chess_Num);
		startPoint = best_move/32;
		EndPoint   = best_move%32;
	}
	...
	for(int i = start; i < start + 7; i++){
		...
		while(p){
			...
			while(dest){
				...
				Result[ResultCount] = src*32+result;
				ResultCount++;
...
double MyAI::Nega_scout(const int* board, const Node bit_board, int* move, const int red_chess_num, const int black_chess_num, const int* cover_chess, const int color, double alpha, double beta, const int depth, const int remain_depth){
	...
	unsigned long long Counter_hash_color =  bit_board.Counter_hash ^ Random_color[color^1];
	unsigned long hash_index = (Counter_hash_color << 32) >> 32;
	...
	if (Hash_table.count(hash_index) > 0){
		...
		if(searched_node.tDepth > remain_depth){
			...
			if (alpha >= beta)
				*move = searched_node.tMove;
				//fprintf(stderr, "return from here 2 %d \n",*move );
				return searched_node.tScore;
		}
	}

	int new_board[32], new_cover[16], new_red, new_black;
	int new_move = -1;
	Node new_bitboard;
	double m = -DBL_MAX;
	...
	if (searched_node.tDepth >= 0){
		...
		*move = searched_node.tMove;
		...
		if (m >= beta){
			...
			//fprintf(stderr, "return from here 3 %d \n",*move );
			History_table[searched_node.tMove] += (1 << searched_node.tDepth);
			return m;
		}
	...
	while(covered_chess){
		...
		covered_chess ^= mask;
		int result = GetIndex(mask);
		Result[move_count] = result*32+result;
		move_count++;
	}
	...
	if(move_count == 0){ // terminal node (no move type)
		...
		//Move ordering
		std::sort(Result, Result + move_count, [this](int x, int y){return cmp(x, y);});
		double n = beta;
		...
		for(int i = 0; i < move_count; i++){ // move
			if (Result[i] == searched_node.tMove)
				continue;
			int t_src = Result[i]/32; int t_dst = Result[i]%32;
			if(t_src != t_dst){
				new_red = red_chess_num, new_black = black_chess_num;
				memcpy(new_board, board, sizeof(int)*32);
				memcpy(new_cover, cover_chess, sizeof(int)*16);
				...
				if(m >= beta){
					...
					new_searched_node.tDepth = remain_depth;
					Hash_table[hash_index] = new_searched_node;
					//fprintf(stderr, "return from here 4 %d \n",*move );
					History_table[*move] += 1ULL << remain_depth;
					return m;
				}
				n = max(alpha, m) + 1;
			}
				...
				double total = 0;
				//chance search
				double A = remain_count * (alpha - Vmax) + Vmax;
				double B = remain_count * (beta - Vmin) + Vmin;
				double m0 = Vmin;
				double M0 = Vmax;
				bool cut = false;
				for(int k = 0; k < remain_count; k++){
					...
					int new_depth = remain_depth - 2 <=  0 ? 0: remain_depth - 2;
					double t = -Nega_scout(new_board, new_bitboard, &new_move, new_red, new_black, new_cover, color^1, -min(B, Vmax), -max(A, Vmin), depth+1, new_depth);
					m0 = m0 + ((t - Vmin) / remain_count);
					M0 = M0 + ((t - Vmax) / remain_count);
				...
				double E_score = cut ? total : (total / remain_total); // calculate the expect value of flip
				if(E_score > m){ 
					m = E_score;
					*move = Result[i];
				}
		...
		if (*move == -1){
			fprintf(stderr, "0 the move never changed \n");
			return m;
		}
		Node_Info new_searched_node;
		if(m <= old_alpha){
		...
		new_searched_node.tDepth = remain_depth;
		Hash_table[hash_index] = new_searched_node;
		//fprintf(stderr, "return from here 6 %d \n",*move );
		History_table[*move] += 1ULL << remain_depth;
		return m;
	}
}
