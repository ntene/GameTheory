...
double UCB(int id){
    ...
    double ucb =  (nodes[id].depth%2) ?  (SR + (C * nodes[parent(id)].sqrtNtotal / nodes[id].sqrtNtotal )) : (1.0 - SR);
    return ucb;
}
    ...
    do {
        ...
        for (myTurn = (start == 'f'); !b.is_terminal(); flip_bit(myTurn)) {
            if (myTurn) {
                ...
                double maxW = nodes[nodes[current].c_id[0]].Scores;
                int maxchild = 0;
                for(int i = 1; i < nodes[current].Nchild; i++){
                    int child = nodes[current].c_id[i];
                    if(nodes[child].Scores > maxW){
                        maxW = nodes[child].Scores;
                        maxchild = child;
                    }
                }
                current = maxchild;
                ...
                flog << "send move ";
                flog << myTurn << " " << move_str << "win rate: " << nodes[maxchild].Scores << " UCB " << UCB(maxchild) <<  std::endl;
